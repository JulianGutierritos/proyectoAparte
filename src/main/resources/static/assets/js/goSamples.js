...
function goSamples() {
	...
	var isExtension = (location.pathname.split('/').slice(-2)[0]
	...
	// footer
	window.hdr = document.createElement("div"); // remember for hiding in
	// goViewSource()
	var p = document.createElement("p");
	p.innerHTML = "<a href='javascript:goViewSource()'>View this sample page's source in-page</a>";
	hdr.appendChild(p);
	var p1 = document.createElement("p");
	var samplename = location.pathname.substring(location.pathname
			.lastIndexOf("/") + 1);
	...
	url = url.slice(lindex + 1).toLowerCase(); // include "/" to avoid matching
	// prefixes
...
function goViewSource() {
	...
	window.hdr.children[0].style.display = "none"; // hide the "View Source"
	// link
